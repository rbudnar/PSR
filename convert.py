...
def get_pixel_count(path, save_files=False):
    ...
    if save_files:
        ...
        save_img(cv2.cvtColor(img_hsv, cv2.COLOR_BGR2RGB),
                 f"{path}_hsv.tif", foldername)
        save_img(cv2.cvtColor(mask_img, cv2.COLOR_BGR2RGB),
                 f"{path}_red_mask.tif", foldername)
        save_img(gray_img, f"{path}_red_mask_count.tif", foldername)
        save_img(cv2.cvtColor(white_mask_img, cv2.COLOR_BGR2RGB),
                 f"{path}_white_mask.tif", foldername)
        save_img(white_mask_img_gray,
                 f"{path}_white_mask_count.tif", foldername)
...
for (i, file) in enumerate(onlyfiles):
    ...
    red_pixel_area, non_tissue_area, total_area, percentage = get_pixel_count(
        file, True)
